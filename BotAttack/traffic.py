...
import statistics
df = pd.read_csv('/Users/harshmodi/Desktop/Hack-O-Mania/cs448b_ipasn.csv')
df['date']= pd.to_datetime(df['date'])
df = df.groupby(['date','l_ipn'],as_index=False).sum()
df['yday'] = df['date'].dt.dayofyear
df['wday'] = df['date'].dt.dayofweek
#This is the code for anomaly detection of bot activity
...
while prob_3[j_3][0] < 2 * prob_3[(j_3)+1][0]:
    ...
    (j_3)+=1    #uses this formula to generate the anomaly days
#get the days as required
...
#now i will show you the graph of the activity
#As you can see the activty is usually high on few days and that will be detected